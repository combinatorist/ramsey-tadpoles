...
def coveredGenerators(generatedSets: Set[Set[Int]], modulo: Int)(s: Seq[Long]) = {
 generatedSets
...
def modDf(modulo: Int) = spark.read.load(s"data/df-overwritten-mod-$modulo").cache

def summary(modulo: Int) = modDf(modulo).as[Seq[Long]].flatMap(cg(modulo)(_).toSeq).groupByKey(_._1).mapValues(_._2).reduceGroups(_ union _).show(false)
